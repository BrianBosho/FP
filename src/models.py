...
# Add this class after GCN
class PE_GCN(torch.nn.Module):
    """
    GCN model with positional encoding support. 
    
    This model can work with or without positional encodings,
    making it a drop-in replacement for the original GCN.
    """
    def __init__(self, dim_in, dim_h, dim_out, pe_dim=None):
        super().__init__()
        self.use_pe = pe_dim is not None
        
        if self.use_pe:
            # If using positional encodings, adjust the input dimension
            self.pe_proj = torch.nn.Linear(pe_dim, dim_h)
            self.feat_proj = torch.nn.Linear(dim_in, dim_h)
            # First GCN layer takes projected combined features
            self.gcn1 = GCNConv(dim_h, dim_h)
        else:
            # Standard GCN if not using positional encodings
            self.gcn1 = GCNConv(dim_in, dim_h)
            
        self.gcn2 = GCNConv(dim_h, dim_out)

    def forward(self, x, edge_index, pe_features=None):
        # Process with or without positional encodings
        if self.use_pe and pe_features is not None:
            # Project original features and positional encodings separately
            feat_proj = self.feat_proj(x)
            pe_proj = self.pe_proj(pe_features)
            
            # Combine them (simple addition works well for this case)
            x = feat_proj + pe_proj
            
            # No dropout before first layer when using PE
            h = self.gcn1(x, edge_index)
        else:
