...
from src.fedgnn.fl.run import load_configuration, main_experiment
...
def run_simple_experiment(args):
    ...
    # Default is repo-local (avoids writing outside the repo).
    # Note: this script uses local imports (run/utils), so keep this simple and relative.
    results_dir = yaml_config.get("results_dir", "runs/simple_experiment")
...
def create_example_config(output_path="simple_config_example.yaml"):
    ...
    example_config = {
        ...
        "data_loading": "full",
        "model": "GCN",
        "results_dir": "runs/simple_experiment",
        "save_results": True,
        "hop": 1,
