...
def save_results_to_csv(results, filename="results.csv"):
    ...
    if exp_dir and timestamp and filename == "results.csv":
        ...
        custom_filename = os.path.join(
            exp_dir, f"training_{experiment_name}_{timestamp}.csv"
        )
...
def compare_model_parameters(server_model, clients, debug=False):
    ...
    all_match = True
    for layer_idx, (server_param, client_params) in enumerate(
        zip(server_params, zip(*client_params_list))
    ):
        ...
        layer_match = all(
            np.array_equal(server_param_np, client_param_np)
            ...
            if not layer_match:
                ...
                for client_idx, client_param_np in enumerate(client_params_np):
                    if not np.array_equal(server_param_np, client_param_np):
                        ...
                        print(
                            f"    Max absolute difference: {np.max(np.abs(server_param_np - client_param_np))}"
                        )
                        print(
                            f"    Mean absolute difference: {np.mean(np.abs(server_param_np - client_param_np))}"
                        )
            else:
...
def generate_experiment_output(
    ...
    test_results,
    client_test_results,
    average_global_results,
    average_client_results,
    std_global,
    std_client,
):
    """
    ...
    with torch.no_grad():
        ...
        if diff_mask.sum() > 0:
            ...
            for idx in diff_indices:
                print(f"Index {idx}:")
                print(
                    f"  True Label: {data.y[data.test_mask][idx]}"
                )
                print(
                    f"  Server Pred: {server_pred[idx]} (Prob: {server_probs[idx].max():.4f})"
                )
                print(
                    f"  Client Pred: {client_pred[idx]} (Prob: {client_probs[idx].max():.4f})"
                )
...
def verify_test_mask(data):
    ...
    unique_classes, class_counts = torch.unique(
        data.y[data.test_mask], return_counts=True
    )
