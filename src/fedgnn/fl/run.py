...
import numpy as np
from src.fedgnn.utils.run import (
    setup_logging,
...
def run_with_server(dataset_name, num_clients, beta, data_loading_option, model_type, cfg, device, hop = 1, fulltraining_flag = False):
    ...
    if _es is not None:
        try:
            from src.fedgnn.fl.train import set_seed as _set_seed
            _set_seed(int(_es))
    ...
    try:
        ...
        for i in range(cfg.get("repetitions", 5)):
            try:
                ...
                from src.fedgnn.utils.memory import clear_memory_aggressive, log_memory_usage
