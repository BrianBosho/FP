...
RESULTS_DIR="$REPO_ROOT/experiments/results/scalability/full_concurrency_sweep"
mkdir -p "$GEN_CFG_DIR" "$LOG_DIR" "$RESULTS_DIR"

TS="$(date +%Y%m%d_%H%M%S)"
SUMMARY="$LOG_DIR/sweep_${TS}_summary.tsv"
...
for n in 2 3 4 5 6 7 8 9 10; do
  if run_one "$n"; then
    ...
    rc=$?
    if [[ $rc -eq 10 ]]; then
      echo "Stopping sweep after N=$n due to scheduling pressure. See $SUMMARY"
    else
      echo "Stopping sweep after N=$n due to failure/OOM. See $SUMMARY"
    fi
    exit 0
  fi
done

echo "Sweep completed through N=10. Summary: $SUMMARY"
