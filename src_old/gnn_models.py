...
class GAT(torch.nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, NumLayers):
        ...
        self.convs = torch.nn.ModuleList()
        self.convs.append(GATConv(nfeat, nhid, heads=8, dropout=dropout))
        for _ in range(NumLayers - 2):
            self.convs.append(GATConv(nhid, nhid, heads=8, dropout=dropout))
        self.convs.append(GATConv(nhid, nclass, heads=8, dropout=dropout))
